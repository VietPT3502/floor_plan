...
from floortrans.loaders import FloorplanSVG, FloorplanPKL
...
def train(args, log_dir, writer, logger):
    ...
    # testloader = data.DataLoader(test_set, batch_size=1, num_workers=num_workers, pin_memory=True)
    # # fix dataset
    # for i, samples in tqdm(enumerate(trainloader), total=len(trainloader), ncols=80, leave=False):
    #     print(samples["image"])
    #     # Save the sample to a file
    #     save_path = "/mnt/hdd/vietpt/convert_cubi/train/sample_" + str(i) + ".pkl"
    #     with open(save_path, 'wb') as f:
    #         pickle.dump(samples, f)
    # for i, samples in tqdm(enumerate(valloader), total=len(valloader), ncols=80, leave=False):
    #     print(samples["image"].shape)
    #     save_path = "/mnt/hdd/vietpt/convert_cubi/val/sample_" + str(i) + ".pkl"
    #     with open(save_path, 'wb') as f:
    #         pickle.dump(samples, f)
    # for i, samples in tqdm(enumerate(testloader), total=len(testloader), ncols=80, leave=False):
    #     save_path = "/mnt/hdd/vietpt/convert_cubi/test/sample_" + str(i) + ".pkl"
    #     with open(save_path, 'wb') as f:
    #         pickle.dump(samples, f)
    # exit()
    ...
    if args.optimizer == 'adam-patience':
        ...
        optimizer = torch.optim.Adam(params, eps=1e-8, betas=(0.9, 0.999))
        scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda epoch: 0.5 ** np.floor(epoch / args.l_rate_drop))
    ...
    if args.weights is not None:
        if os.path.exists(args.weights):
            ...
            logger.info(f"Loaded checkpoint '{args.weights}' (epoch {checkpoint['epoch']})")
        else:
    ...
    for epoch in range(args.n_epoch):
        ...
        for i, samples in tqdm(enumerate(trainloader), total=len(trainloader), ncols=80, leave=False):
            ...
            losses = losses._append(criterion.get_loss(), ignore_index=True)
            variances = variances._append(criterion.get_var(), ignore_index=True)
            ss = ss._append(criterion.get_s(), ignore_index=True)

            optimizer.zero_grad()
            loss.backward()
            # t5 = time.time()
            # print("backward ", t5 -t4)
            optimizer.step()
            # t1 = time.time()
       ...
       # Validation
        ...
        for i_val, samples_val in tqdm(enumerate(valloader), total=len(valloader), ncols=80, leave=False):
            with torch.no_grad():
                ...
                val_losses = val_losses._append(criterion.get_loss(), ignore_index=True)
                val_variances = val_variances._append(criterion.get_var(), ignore_index=True)
                val_ss = val_ss._append(criterion.get_s(), ignore_index=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n-classes', nargs='?', type=int, default=97,
                        help='# of the epochs')
